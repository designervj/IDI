        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {cards.map((card, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              whileHover={{ y: -10 }}
              className="bg-white p-10 rounded-[10px] shadow-xl shadow-gray-200/50 border border-gray-100 flex flex-col items-center text-center group"
            >
              <div className={`w-20 h-20 ${card.color} rounded-[10px] flex items-center justify-center mb-8 shadow-lg group-hover:rotate-6 transition-transform duration-300`}>
                {card.icon}
              </div>
